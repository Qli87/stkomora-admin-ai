/**
 * View, Download, Print, Remove for a single consultant contract.
 */
...
export default function ConsultantContractActions({
  consultantId,
  contract,
  onRemoved,
  showRemove = false,
  ...
  const handleView = async () => {
    setLoading('view');
    try {
      const blob = await externalConsultantsApi.fetchContractBlob(
        consultantId,
        contract.id,
        'inline'
      );
  ...
  const handleRemove = async () => {
    setLoading('remove');
    try {
      await externalConsultantsApi.removeContract(consultantId, contract.id);
